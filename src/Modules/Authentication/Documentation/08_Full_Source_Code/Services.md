# Full Source Code — Services

## `src/Modules/Authentication/Application/Services/AuthenticationService.php`
...
class AuthenticationService
{
    ...
    {
        $this->registrationRequestService->markAsCreatedByEmail($email, $userId);
    }

    public function requestUserRegistration(array $data): array
    {
        $existingRequest = $this->repository->findPendingRegistrationRequestByEmail($data['email']);

        if ($existingRequest) {
            throw new BusinessLogicException('A registration request with this email is already pending.');
        }

        $existingUser = $this->repository->findUserByEmail($data['email']);

        if ($existingUser) {
            throw new BusinessLogicException('This email is already associated with an existing account.');
        }

        $this->repository->createRegistrationRequest([
            'username' => $data['username'],
            'email' => $data['email'],
        ]);

        $admins = Authentication::role('admin')->get();
        $requestedAt = now()->toDateTimeString();

        foreach ($admins as $admin) {
            Mail::to($admin->email)->send(
                new RegistrationRequestMail($data['username'], $data['email'], $requestedAt)
            );
        }
...
## `src/Modules/Authentication/Application/Services/UserRegistrationRequestService.php`
